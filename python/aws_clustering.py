...
def main():
    ...
    try:
        ...
        sys.stdout.write("create the instance as peer\n")
        for i in range(peer):
            instance_name = "%s_%s_%s" % (prefix, "peer", i)
            sys.stdout.write("[%s] create the peer with name #%s#\n" % ("peer", instance_name))
            # aws_instances.append(create_instance("peer", instance_name))
